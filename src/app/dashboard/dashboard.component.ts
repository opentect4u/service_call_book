...
const GET_CLIENT_CLOSED_TICKITES_DETAILS = gql`
query clientClosedTkt($client_id:String!){
  clientClosedTkt(client_id:$client_id){
    id
    tkt_no
    tktStatus
    work_status
...
export class DashboardComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.apollo.watchQuery<any>({
      ...
      this.employee_phone = data.clientLastTke[0]?.emp_phone_no;
  ...
  getClosedTickets() {
    this.apollo.watchQuery<any>({
      query: GET_CLIENT_CLOSED_TICKITES_DETAILS,
      variables: {
        client_id: localStorage.getItem('UserId')
      },
      pollInterval: 40000,
      ...
      this.lastTwoCompleted = data.clientClosedTkt.slice(-2);
    })
  }

  closeTicket(ticket: any) {
    // Placeholder for opening modal to add remarks and set permanent close flag
