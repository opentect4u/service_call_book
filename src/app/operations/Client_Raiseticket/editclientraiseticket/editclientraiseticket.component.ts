...
const CLOSE_CLIENT_TKT = gql`
mutation clientCloseTkt(
  $id:String!,
  $client_id: String!,
  $client_close_flag: String!,
  $client_closing_remarks: String!,
  $client_closed_rating: String!,
  $user_id: String!
  ) {
    ...
    clientCloseTkt(
      id:$id,
      client_id:$client_id,
      client_close_flag: $client_close_flag,
      client_closing_remarks: $client_closing_remarks,
      client_closed_rating: $client_closed_rating,
      user_id:$user_id
      )  {
...
export class EditclientraiseticketComponent implements OnInit {
  ...
  remarks: any;
  email: any;
  isClientColosedTicket: any
  closedDateTime: any
  closingRemarks: any
  closingExperience: any
  ...
  getClientDetails() {
    this.apollo.watchQuery<any>({
      ...
      .subscribe(({ data }) => {
        ...
        console.log(this.tkt_module);
        $('select').select2().select2('val', this.tkt_module)
        this.work_status = data.clientGetTkt[0].work_status;
        this.isClientColosedTicket = data.clientGetTkt[0].closed_by_client;
        this.closedDateTime = data.clientGetTkt[0].closed_by_client_datetime;
        this.closingRemarks = data.clientGetTkt[0].client_closing_remarks;
        this.closingExperience = data.clientGetTkt[0].client_closed_rating;
      })
  }
  closeTicketByClient(date: any, client_id: any, client_email: any, eng_name: any, cl_remarks: any, rating: any){
    console.log(date, client_id, client_email, eng_name, cl_remarks, rating);
    this.apollo.mutate({
      mutation: CLOSE_CLIENT_TKT,
      variables: {
        client_id: localStorage.getItem('UserId'),
        id: this.Id,
        client_close_flag: 'Y',
        client_closing_remarks: cl_remarks,
        client_closed_rating: rating,
        user_id: localStorage.getItem("UserId")
      }
    }).subscribe(({ data }) => {
      console.log(data);
      this.edit = data;
      if (this.edit.clientCloseTkt.success == 1) {
        var now = new Date();
        var time = "06:30 PM";
        var getModDateTime = (now.getMonth() + 1) + "/" + now.getDate() + "/" + now.getFullYear() + " " + time;
        var userval = new Date(getModDateTime);
